...
def rc_HMM(df, coverage, fetal_fraction, read_count_params):
    ...
    # return DataFrame
    df = pd.DataFrame({"sample" : df["sample"].values,
                       "state"  : states
                       })
    return(df)
...
def rc_HMM2(df, coverage, read_count_params):
    ...
    coverage = 500 if coverage // 500 == 0 else coverage // 500 * 500  # add for experiment
    rc = read_count_params[(read_count_params["coverage"] == coverage) & (read_count_params["fetal_fraction"] == fetal_fraction)]
...
def ar_HMM(df, coverage, fetal_fraction, allelic_ratio_params):
    ...
    eup_prob = eup_ssr / eup_ssr.sum()
    tri_prob = tri_ssr / tri_ssr.sum()
    transmat = np.array([eup_prob,  # state 1 - euploidy
    ...
    # read params
    ar = allelic_ratio_params[(allelic_ratio_params["coverage"] == coverage) &
    ...
    means = np.array([[ar1_mean],   # state 1 - euploidy
    ...
    covars = np.array([[[ar1_var]],   # state 1 - euploidy
...
def rcar_HMM(df, coverage, fetal_fraction, allelic_ratio_params, read_count_params):
    # start probability
    startprob = np.repeat(1 / 7, 7)
    ...
    ar = allelic_ratio_params[(allelic_ratio_params["coverage"] == coverage) & 
                              (allelic_ratio_params["fetal_fraction"] == fetal_fraction)]
    rc = read_count_params[(read_count_params["coverage"] == coverage) &
    ...
    means = np.array([[ar1_mean, rc_eup_mean],  # state 1 - euploidy
    ...
    covars = np.array([[[ar1_var, 0.],       # state 1 - euploidy
                        [0., rc_eup_var]],
                       [[ar2_var, 0.],       # state 2 - trisomy
                        [0., rc_tri_var]],
                       [[ar3_var, 0.],       # state 3 - euploidy
                        [0., rc_eup_var]],
                       [[ar4_var, 0.],       # state 4 - paternal trisomy
                        [0., rc_tri_var]],
                       [[ar5_var, 0.],       # state 5 - paternal trisomy
                        [0., rc_tri_var]],
                       [[ar6_var, 0.],       # state 6 - euploidy
                        [0., rc_eup_var]],
                       [[ar7_var, 0.],       # state 7 - trisomy
                        [0., rc_tri_var]]])
